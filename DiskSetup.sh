...
function CarveLVM_Standard {
    ...
    parted -s "${CHROOTDEV}" -- mktable gpt \
        mkpart primary 2048s "${BOOTBLKSZ}MiB" \
        mkpart primary "${FSTYPE}" "${BOOTBLKSZ}MiB" "${BOOTDEVSZ}MiB" \
...
OPTIONBUFR=$( getopt \
    -o b:B:d:f:hl:p:r:v: \
    --long bootlabel:,bootblk-size:,bootprt-size:,disk:,fstype:,help,label-boot:,partition-string:,rootlabel:,vgname: \
    -n "${PROGNAME}" -- "$@")
